"""
Simplified reward utilities for mathematical answer evaluation.

This file is adapted from deepscaler: https://github.com/rllm-org/rllm.
"""
...
def mathd_normalize_answer(answer: Optional[str]) -> Optional[str]:
    """Normalize answer using MATH dataset conventions."""
    if answer is None:
        return None
    answer = answer.strip()
    try:
...
def _strip_string(string):
    """Strip and normalize LaTeX string."""
...
def _strip_properly_formatted_commas(expr: str):
    """Strip commas from numbers while being careful not to strip tuple commas."""
...
def should_allow_eval(expr: str):
    """Check if expression should be evaluated with sympy."""
...
def extract_answer(passage: str) -> str:
    ...
    if "\\boxed" in passage:
        solution = last_boxed_only_string(passage)
...
def grade_answer_sympy(given_answer: str, ground_truth: str) -> bool:
    """Grade answer using sympy-based normalization and comparison."""
...
def grade_answer_mathd(given_answer: str, ground_truth: str) -> bool:
    """Grade answer using MATH dataset normalization."""
...
def grade_answer_verl(solution_str, ground_truth):
    """Grade answer using combined mathd and sympy methods."""
    if not ground_truth:
        return False
...
# ============================================================================
# Simplified Math Reward Function
# ============================================================================

def rllm_reward_fn_math(
    data_source: str,
    llm_solution: str,
    ...
    **kwargs
    ...
    Args:
        ...
        extra_info: Additional metadata (currently unused, kept for compatibility)
        require_think_end: If True, require </think> tag in solution
        strip_comma_from_answer: If True, strip commas from the answer before grading
        **kwargs: Additional options (currently unused, kept for compatibility)
    ...
    # Extract solution after </think> tag
    if "</think>" in llm_solution:
        model_solution = llm_solution.split("</think>")[1]
    else:
        if require_think_end:
            return 0.0  # Format error
        else:
            model_solution = llm_solution
    ...
    if strip_comma_from_answer:
        model_answer = model_answer.replace(",", "")

    # Handle ground truth(s)
