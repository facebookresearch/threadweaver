#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="."
ROOT_DIR="."

OPENAI_KEY_PATH=${OPENAI_KEY_PATH:-~/.openai_api_key}
if [ -f "$OPENAI_KEY_PATH" ]; then
  export OPENAI_API_KEY=$(cat "$OPENAI_KEY_PATH")
elif [ -z "${OPENAI_API_KEY:-}" ]; then
  echo "OPENAI_API_KEY is not set and OPENAI_KEY_PATH does not exist at $OPENAI_KEY_PATH" >&2
  exit 1
fi
STEP2_VERSION=${STEP2_VERSION:-v1}
VERSION=${STEP2_VERSION}_v1

NUM_SAMPLES=${NUM_SAMPLES:-20}
SRC_NAME=${SRC_NAME:-Qwen3_8B_bfloat16_fixed_qwen_template_40k_polaris_data_53K_1_1k}
INPUT_SUFFIX=${INPUT_SUFFIX:-}
DIR_NAME=${DIR_NAME:-${SRC_NAME}${INPUT_SUFFIX}_${NUM_SAMPLES}samples}
DATA_PATH=${ROOT_DIR}/data/${DIR_NAME}
SUFFIX=${SUFFIX:-}

python "$ROOT_DIR/src/rewrite-context_v1.py" \
  --prompt "$ROOT_DIR/prompt/step3-prompt_${VERSION}.txt" \
  --input "${DATA_PATH}_step2_${STEP2_VERSION}${SUFFIX}" \
  --output "${DATA_PATH}_step3_${VERSION}${SUFFIX}" \
  --glob "*.txt" \
  --openai_model gpt-5-2025-08-07 \
  --workers 128 \
  --in_cost_per_million 1.25 \
  --out_cost_per_million 10.0 \
  --effort "${EFFORT:-medium}" \
  --trace "${DATA_PATH}_step3_${VERSION}${SUFFIX}/trace"
